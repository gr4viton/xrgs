...
public:
  void RequestHapticFeedback(ControllerEnum index, float amplitude01 = 0.3f, int64_t duration = XR_MIN_HAPTIC_DURATION, float frequency = XR_FREQUENCY_UNSPECIFIED)
  {
    HapticsData hapticsData = {.isActive = true, .amplitude01 = amplitude01, .duration = duration, .frequency = frequency};
  ...
  const std::vector<HapticsData>& GetHapticFeedbackRequests() { return hapticsDataVect; }
  void                            ClearHapticFeedbackRequests() { hapticsDataVect.clear(); }
};
}  // namespace Inputspace